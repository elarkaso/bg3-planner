...
function parseEvent(text) {
  ...
  // volitelný prefix +N
  // zbytek: den + start-end + titulek
  const m = cleaned.match(/^(?:\+(\d+)\s+)?(\S+)\s+(\d{1,2})\s*[-–]\s*(\d{1,2})\s*(.*)$/i);
...
export async function POST(req) {
  ...
  if (interaction.type !== 2 || interaction.data?.name !== "bg3") {
    return json({ type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE, data: { content: "Použij `/bg3 event: so 20-24 raid`" } });
  }
  ...
  const createdBy = interaction.member?.user?.username || interaction.user?.username || "discord";
  ...
  return json({
    type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
    data: {
  content: `✅ Uloženo (týden ${weekKey}): **${parsed.title}** — ${dn[parsed.day]} ${String(parsed.startHour).padStart(2,"0")}:00–${String(parsed.endHour).padStart(2,"0")}:00`, },  
    });
}